var SUPABASE_URL=SKYVAYU_CONFIG.supabaseUrl;var SUPABASE_KEY=SKYVAYU_CONFIG.supabaseKey;var userToken=SUPABASE_KEY;
...
function fmtDate(d){if(!d)return'—';try{var dt=new Date(d);if(isNaN(dt.getTime()))return'—';return dt.toLocaleDateString('en-IN',{day:'2-digit',month:'short',year:'numeric'});}catch(e){return'—';}}
...
async function loadAllData(){
  ...
  var results=await Promise.all([
    (function(){ var cat = currentUser.aircraft_category||currentOperator.aircraft_category; var url = 'queries?status=eq.open' + (cat ? '&aircraft_category=in.(' + cat + ')' : '') + '&order=created_at.desc'; return sbFetch(url); })(),
...
function renderActiveList(queries){
  ...
  el.innerHTML=queries.map(function(q){
    var r=q.trip_type==='multi'?'Multiple sectors':escapeHtml(q.departure||'-')+' ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ '+escapeHtml(q.destination||'-');
    ...
    if(lockedBySomeoneElse){
      ...
      lockInfo='<div class="query-lock-info" data-query="'+escapeHtml(q.id)+'" data-expires="'+escapeHtml(claim.expires_at)+'">Locked by '+escapeHtml(claim.claimed_by_name||'teammate')+' ÃÂÃÂÃÂÃÂ· '+(rem||'expiring')+'</div>';
    }else if(lockedByMe){
      lockInfo='<div class="query-lock-info" style="color:var(--green-light);" data-query="'+escapeHtml(q.id)+'" data-expires="'+escapeHtml(claim.expires_at)+'">You have this locked ÃÂÃÂÃÂÃÂ· '+(claimRemaining(claim.expires_at)||'expiring')+'</div>';
    }
...
function viewSharedQuote(quoteId) {
  ...
  var route = query.trip_type === 'multi' ? 'Multiple sectors'
    : escapeHtml(query.departure || '-') + ' ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ' + escapeHtml(query.destination || '-');
...
function renderConfirmedList(quotes){
  ...
  el.innerHTML=quotes.map(function(q){
    ...
    var route=query.trip_type==='multi'?'Multiple sectors':escapeHtml(query.departure||'-')+' ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ '+escapeHtml(query.destination||'-');
    var empBadge='';
    if(isOwner()){var u=lookupUser(q.submitted_by);empBadge=u?'<span class="badge badge-by">by '+escapeHtml(u.full_name||u.username)+'</span>':'';}    return '<div class="query-card"><div class="query-top"><div><div class="query-route">'+route+empBadge+'</div><div class="query-meta">'+fmtDate(query.flight_date)+(query.flight_time?' at '+escapeHtml(query.flight_time):'')+' ÃÂÃÂÃÂÃÂ· '+escapeHtml(q.aircraft_type||'')+(q.aircraft_registration?' ('+escapeHtml(q.aircraft_registration)+')':'')+'</div></div><span class="badge badge-accepted">Confirmed</span></div><div class="query-details"><div class="query-detail"><span>Pax</span>'+escapeHtml(String(query.passengers||'-'))+'</div><div class="query-detail"><span>Revenue</span>'+fmtPrice(q.price)+'</div></div></div>';
  }).join('');
}
...
function updateClaimTimers(){
  document.querySelectorAll('.query-lock-info').forEach(function(el){
    ...
    var prefix=el.textContent.split(' ÃÂÃÂÃÂÃÂ· ')[0];
    el.textContent=prefix+' ÃÂÃÂÃÂÃÂ· '+rem;
  });
}
...
function getBusyAircraftMap(){
  ...
  allMyOperatorQuotes.forEach(function(q){
    ...
    dates.forEach(function(d){
      ...
      var u=lookupUser(q.submitted_by);
      map[q.aircraft_id][d]={by:u?(u.full_name||u.username):'operator',route:(query.departure||'-')+'ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ'+(query.destination||'-')};
    });
  });
  return map;
}
...
async function openQuoteModal(queryId){
  ...
  var route=query.trip_type==='multi'?'Multiple sectors':escapeHtml(query.departure||'ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ')+' ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ '+escapeHtml(query.destination||'ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ');
...
function openDocRenewModal(acId, docKey, docName, expKey, urlKey, nameKey){
  ...
  document.getElementById('doc-renew-aircraft-info').textContent = ac ? ac.aircraft_type+' ÃÂÃÂÃÂÃÂ· '+ac.registration : '';
  ...
  ['am-wifi','am-oven','am-microwave','am-coffee','am-flatbed','am-lavatory','am-satphone','am-entertainment','am-nonsmoking'].forEach(function(id){var el=document.getElementById(id);if(el)el.checked=false;});
    dates.forEach(function(d,idx){
      ...
      if(booking){
        ...
        cellContent='<div class="'+bClass+'" title="'+booking.route+' ÃÂÃÂÃÂÃÂ· by '+booking.by+'"><span class="b-route">'+routeTxt+'</span><span class="b-emp">'+bLabel+'</span></div>';
      }
...
async function loadEmployees(){
  ...
  var rows=emps.map(function(e){
    ...
    var html='<tr onclick="toggleEmployee(\''+escapeHtml(e.id)+'\')"><td><span class="emp-avatar">'+escapeHtml(ini)+'</span><b>'+escapeHtml(e.full_name||e.username)+'</b>'+pendingBadge+'<br><span style="font-size:11px;color:var(--text-tertiary);">@'+escapeHtml(e.username)+(e.employee_id?' ÃÂÃÂÃÂÃÂ· ID: '+escapeHtml(e.employee_id):'')+'</span></td>'
    ...
    if(isExpanded){
      var activityHtml=s.activity.slice(0,15).map(function(q){
        var qq=q.queries||{};
        var route=escapeHtml(qq.departure||'-')+'ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ'+escapeHtml(qq.destination||'-');
        ...
        return'<div class="emp-log-row"><span class="emp-log-time">'+when+'</span><span class="emp-log-action"><b>'+statusLabel+'</b> ÃÂÃÂÃÂÃÂ· '+route+' ÃÂÃÂÃÂÃÂ· '+fmtPrice(q.price)+' ÃÂÃÂÃÂÃÂ· '+escapeHtml(q.aircraft_type||'')+'</span></div>';
      }).join('');
...
function saveCategorySettings(){
  ...
  sbFetch('operator_users?id=eq.'+currentUser.id, {method:'PATCH', body:{aircraft_category:cats}})
    .then(function(res){
      if(res.ok){
        currentUser.aircraft_category = cats;
...
function loadProfileCategory() {
  if (!currentOperator) return;
  var cats = currentUser.aircraft_category || currentOperator.aircraft_category || '';
...
async function loadProfileSection() {
  ...
  // ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ Company card (owner only) ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ
